...
class JumpPointSearchPlanner:
    ...
    def __init__(self, config: JPSPlannerConfig, logger: logging.Logger):
        self.config = config
        self.logger = logger
        self.grid: np.ndarray | None = None          # Трехмерная сетка 0 свободно, 1 препятствие
    ...
    def _get_natural_and_forced_neighbors(self, current_pos: tuple[int,int,int], parent_pos: tuple[int,int,int] | None) -> list[tuple[int,int,int]]:
        ...
        if parent_pos is None:
            for dx_n in [-1, 0, 1]:
                for dy_n in [-1, 0, 1]:
                    for dz_n in [-1, 0, 1]:
                        if dx_n == 0 and dy_n == 0 and dz_n == 0: 
                            continue # Исключаем сам узел
        ...
        # Определяем направление, по которому мы пришли в текущий узел
            ...
            # Аналогично для XZ грани (dir_y = 0)
                ...
                for dy_f in [-1, 1]:
                    if self._is_free(px, py + dy_f, pz) and ( \
                       ...
                       self._is_obstacle(px, py + dy_f, pz - dir_z) \
                    ):
                        neighbors_directions.append((0,dy_f,0))
            ...
            # Аналогично для YZ грани (dir_x = 0)
                ...
                for dx_f in [-1, 1]:
                    if self._is_free(px + dx_f, py, pz) and ( \
                       ...
                       self._is_obstacle(px + dx_f, py, pz - dir_z) \
                    ):
                        neighbors_directions.append((dx_f,0,0))
                        if self._is_free(px+dx_f, py+dir_y, pz): neighbors_directions.append((dx_f,dir_y,0))
                        if self._is_free(px+dx_f, py, pz+dir_z): neighbors_directions.append((dx_f,0,dir_z))
        ...
        # --- ГЛАВНАЯ ДИАГОНАЛЬНОЕ ДВИЖЕНИЕ ---
            ...
            # Проверка, является ли компонентная грань YZ вынужденной (из-за блокировки позади X)
    ...
    def _jump(self, current_x: int, current_y: int, current_z: int,
        ...
        # 1. Проверка на выход за пределы сетки или столкновение с препятствием
        if not self._is_free(next_x, next_y, next_z):
            return None

        # 2. Проверка, является ли следующая ячейка целью
        if (next_x, next_y, next_z) == self.goal_pos_grid:
            return self.goal_pos_grid

        # 3. Проверка на наличие Forced Neighbors (вынужденных соседей)
        
        # --- Проверка для ОСЕВОГО движения ---
        if dx != 0 and dy == 0 and dz == 0:
            # Проверяем forced neighbors в Y-направлении:
            ...
            # Проверяем forced neighbors в Z-направлении (аналогично)
        ...
        # Движение по грани (XY, XZ, YZ)
        if dx != 0 and dy != 0 and dz == 0: # XY-plane (движение в XY, Z константа)
        ...
        elif dx != 0 and dz != 0 and dy == 0: # XZ-plane (движение в XZ, Y константа)
            if self._jump(next_x, next_y, next_z, dx, 0, 0) is not None or \
        ...
        elif dy != 0 and dz != 0 and dx == 0: # YZ-plane (движение в YZ, X константа)
        ...
        # 4. Если не нашли Jump Point, продолжаем прыжок в том же направлении.
    ...
    def _identify_successors(self, current_node: NodeJPS) -> list[NodeJPS]:
        ...
        """
        successors = []
        # Определяем родительскую позицию для правил прореживания JPS
        parent_pos = current_node.parent.position if current_node.parent else None
        
        # Получаем набор направлений, которые не были бы отброшены JPS-прореживанием
    ...
    def plan(self, start_xyz_world: np.ndarray, goal_xyz_world: np.ndarray,
        ...
        # 1. Определение границ сетки на основе старта, цели и препятствий
        ...
        for obs in obstacles_world_3d:
            # Учитываем инфляцию препятствий радиусом робота для определения границ сетки
            inflated_r_xy = obs['radius_xy'] + robot_r_jps
            inflated_h_half_z = obs['half_height_z'] + robot_r_jps
        ...
        self.grid = np.zeros(self.grid_shape, dtype=np.uint8) # Создаем пустую сетку

        # Вспомогательная функция для преобразования мировых координат в координаты сетки
        def world_to_grid_xyz(wx: float, wy: float, wz: float) -> tuple[int, int, int]:
            ...
            # Убеждаемся, что координаты не выходят за границы сетки
            return max(0, min(gx, self.grid_shape[0] - 1)), \
                   max(0, min(gy, self.grid_shape[1] - 1)), \
                   max(0, min(gz, self.grid_shape[2] - 1))

        # 2. Вокселизация препятствий (инфляция радиусом робота)
        for obs in obstacles_world_3d:
            obs_center_w, obs_r_xy, obs_h_half_z = obs['pos_center'], obs['radius_xy'], obs['half_height_z']
            inflated_r_xy = obs_r_xy + robot_r_jps
            inflated_obs_z_min_w = obs_center_w[2] - obs_h_half_z - robot_r_jps
            inflated_obs_z_max_w = obs_center_w[2] + obs_h_half_z + robot_r_jps
            
            # Определяем границы вокселей, которые нужно пометить как препятствия
            ...
            for igx in range(min_gx_obs, max_gx_obs + 1):
                for igy in range(min_gy_obs, max_gy_obs + 1):
                    # Вычисляем мировые XY-координаты центра текущей ячейки
                    ...
                    # Проверяем расстояние в XY-плоскости
                    if (cell_center_xy_w_x - obs_center_w[0])**2 + (cell_center_xy_w_y - obs_center_w[1])**2 <= inflated_r_xy**2:
                        for igz in range(min_gz_obs_approx, max_gz_obs_approx + 1):
                            # Вычисляем мировые Z-координаты центра текущей ячейки
                            cell_center_z_w = self.grid_origin_world[2] + (igz + 0.5) * self.resolution
                            # Проверяем Z-диапазон
                            if inflated_obs_z_min_w <= cell_center_z_w <= inflated_obs_z_max_w:
                                if self._is_valid(igx,igy,igz): # Убеждаемся, что ячейка в границах
                                    self.grid[igx, igy, igz] = 1 # Помечаем как препятствие

        # 3. Подготовка к поиску пути JPS (A* с оптимизацией JPS)
        ...
        # Проверка, не находится ли стартовая позиция в препятствии
        if self._is_obstacle(*start_node.position):
            ...
            # Поиск ближайшей свободной ячейки (поиск в радиусе 1-3 ячеек)
...
class SimpleController(Controller):
    def __init__(self, node: Node, steps: list[Step], origin_pos_abs: np.ndarray, control_frequency: float = 20.0):
        ...
        self.jps_planner_config.robot_radius_jps = self._motion_config.robot_radius + 0.3 # Чуть больший радиус для инфляции JPS
        ...
        self.node.get_logger().info("SimpleController (JPS) инициализирован.")
    ...
    def obstacles_callback(self, msg: String):
        ...
        if len(parts) > 0 and len(parts) % 7 == 0:
            ...
            if current_obstacle_summary != old_obstacle_summary:
                ...
                self.node.get_logger().warn(f"{YELLOW}OBSTACLES UPDATED! Count: {num_obs}. Details: {'; '.join(obs_summary_list)}{RESET}. Replanning will be triggered if in JPS Nav.")
        elif msg.data.strip():
            self.node.get_logger().warn(f"Invalid obstacle message format: '{msg.data}'. Clearing obstacles.")
            if self._detected_obstacles_global:
                self._detected_obstacles_global = []
                self.new_obstacles_for_replan = True
...
class JPSNavToGlobalGoal(Step):
    ...
    def _plan_path_jps(self, controller: Controller, start_pos_local_odom: np.ndarray) -> bool:
        ...
        for obs_global in controller.detected_obstacles_global:
            ...
            obstacles_local_odom_3d.append({
        ...
        for obs_local in obstacles_local_odom_3d:
            ...
            if dist_xy_sq < effective_obs_radius_xy**2:
                obs_z_min = obs_local['pos_center'][2] - obs_local['half_height_z'] - controller.motion_config.robot_radius
                obs_z_max = obs_local['pos_center'][2] + obs_local['half_height_z'] + controller.motion_config.robot_radius
    ...
    def _update_lookahead_target2(self,controller: Controller, current_pos_local_odom: np.ndarray):
        """
        Обновляет look-ahead точку, которую дрон должен преследовать.
        Эта точка находится на lookahead_distance впереди дрона по JPS-пути.
        """
        ...
        for i in range(len(self._jps_path_local_odom)):
            ...
            if dist_sq < min_dist_sq:
                min_dist_sq = dist_sq
                closest_idx = i
        self._current_path_segment_idx = max(self._current_path_segment_idx, closest_idx)

        if self._current_path_segment_idx >= len(self._jps_path_local_odom) - 1:
            self._current_lookahead_target = self.final_target_pos_local_odom
            return

        target_distance_along_path_from_start = self._path_segment_lengths[self._current_path_segment_idx] + self.lookahead_distance

        lookahead_segment_start_idx = self._current_path_segment_idx
        while lookahead_segment_start_idx < len(self._path_segment_lengths) - 1 and \
              self._path_segment_lengths[lookahead_segment_start_idx + 1] < target_distance_along_path_from_start:
            lookahead_segment_start_idx += 1

        if lookahead_segment_start_idx >= len(self._jps_path_local_odom) - 1:
            self._current_lookahead_target = self.final_target_pos_local_odom
            return

        p1 = self._jps_path_local_odom[lookahead_segment_start_idx]
        p2 = self._jps_path_local_odom[lookahead_segment_start_idx + 1]
        
        segment_start_cumulative_dist = self._path_segment_lengths[lookahead_segment_start_idx]
        segment_length = np.linalg.norm(p2 - p1)

        if segment_length > 1e-3:
            t = (target_distance_along_path_from_start - segment_start_cumulative_dist) / segment_length
            t = np.clip(t, 0.0, 1.0)
            self._current_lookahead_target = p1 + (p2 - p1) * t
        else:
            self._current_lookahead_target = p2

        if np.linalg.norm(self._current_lookahead_target - self.final_target_pos_local_odom) < self.final_tolerance_xy * 2:
            self._current_lookahead_target = self.final_target_pos_local_odom
            
        controller.logger.info(f"JPSNav: Lookahead update. Current drone pos: {vector_to_str(current_pos_local_odom)}")
        controller.logger.info(f"JPSNav: Path segment idx: {self._current_path_segment_idx}. Path lengths: {[f'{l:.1f}' for l in self._path_segment_lengths]}")
        controller.logger.info(f"JPSNav: Lookahead distance: {self.lookahead_distance:.1f}. Target distance along path: {target_distance_along_path_from_start:.1f}")
        controller.logger.info(f"JPSNav: Chosen lookahead target: {vector_to_str(self._current_lookahead_target)}")
    ...
    def update(self, controller: Controller) -> bool:
        ...
        controller.logger.info(f"JPSNav: Current Pos: {vector_to_str(current_pos_local_odom)}. Path has {len(self._jps_path_local_odom)} JPs.")
...
class Land(Step):
    ...
    def init(self, controller: Controller) -> None:
        ...
        if controller.viz_publisher: controller.clear_jps_path_markers() # Очистка маркеров пути JPS при посадке
...
def main(args=None):
    ...
    landing_target_z_local_odom = target_waypoints_local_odom[-1][2] + 0.1 
